...
prev_finger = []
def prevs_finger(closest_finger):
    prev_finger.append(closest_finger)
    return prev_finger
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            print(f"Detected finger: {save_finger}")

            
            for i in range(21):
                cv2.putText(frame, f"{i}", (int(hand_landmarks.landmark[i].x * frame.shape[1]), int(hand_landmarks.landmark[i].y * frame.shape[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)


            # Display the detected finger on the frame
            cv2.putText(frame, f"Finger: {save_finger}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            save_finger = prevs_finger(closest_finger[0])
