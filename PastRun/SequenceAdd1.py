...
# (Optional) Gesture Pattern function – not used in this main code but kept for reference.
def gesture_pattern(finger_sequence):
    # Example: Prepend a base pattern and then add detected finger names.
    # (You might want to adjust the formatting.)
    pattern = "'1','2','3','2','1'"
    for finger in finger_sequence:
        pattern += f",'{finger[0]}'"
    return pattern

def pattern_classification(finger_sequence):
    pattern = gesture_pattern(finger_sequence)
    results = SequenceCollection.get(include=["embeddings", "metadatas"])
    embeddings = results["embeddings"]
    metadatas = results["metadatas"]
    for i, embedding in enumerate(embeddings):
        distance = euclidean_distance(pattern, embedding)  # Note: This line is questionable if pattern is a string!
        if distance < 0.5:
            return metadatas[i]["gesture"]
    return "Unknown"

# (Unused) Remove or update if you plan to use it.
# def gesture_pattern_time(finger_sequence):
#     pattern = ""
#     for finger in finger_sequence:
#         pattern += finger[0]
#     return pattern
